...
<script type="text/javascript">
    ...
    (function() {
        ...
        $(document).ready(function(){
            ...
            $("#validationbuscarCliente1").typeahead('val', "{!! $venta->cliente->NombreCompleto !!}");
            $('#VentaServicios input[name=\"IdCliente\"]').val({!! $venta->IdCliente !!})
...
<form action="{{route('ventasservicios.update',$venta->IdVentaServicio)}}" method="POST" id="VentaServicios"  class="needs-validation" novalidate>
    @csrf
    @method('put')
    <div class="tab-content" id="pills-tabContent">
            <div class="tab-pane fade show active" id="pills-general" role="tabpanel" aria-labelledby="pills-general-tab">
                ...
                <div class="form-group col-md-6">
                    ...
                    <input type="search" name="NombreCliente" class="form-control typeahead" placeholder="Cliente" autocomplete="off" id="validationbuscarCliente1"  >

{{--                    <script type="javascript">--}}
{{--                        $("#validationbuscarCliente1").typeahead('val', "Gregorio Waters");--}}
{{--                        var myVal =  $("#validationbuscarCliente1").typeahead('val');--}}
{{--                        console.log(myval);--}}
{{--                    </script>--}}


                    <input type="hidden" name="IdCliente"   value="{{$venta->cliente->IdCliente}}" required />
                ...
                <div class="form-group col-md-6">
                    ...
                    <input type="date" name="FechaHoraVenta" id="FechaHoraVenta" class="form-control"  required  value="{{ \Carbon\Carbon::parse($venta->FechaHoraVenta )->format('Y-m-d')}}" readonly>
                </div>
...
{{--                    <input type="input" name="CodigoEstadoVenta" id="CodigoEstadoVenta" class="form-control"  required  value="INICIADO" readonly>--}}
                    <select id="CodigoEstadoVenta" class="form-control" name="CodigoEstadoVenta" >
                        ...
                        @if($venta->CodigoEstadoVenta=='A')
                            <option value="I" selected>ANULADO</option>
                        @else
                            <option value="I">ANULADO</option>
                        @endif

                        @if($venta->CodigoEstadoVenta=='F')
                            <option value="I" selected>FINALIZADO</option>
                        @else
...
{{--                </div>--}}
                ...
                <div class="form-group col-md-6">
                    ...
                    <input type="text" class="form-control" id="NroCasillero" placeholder="Casillero" name="NroCasillero" required value="{{$venta->NroCasillero}}">
                    <div class="invalid-feedback">
...
{{--                                        "</tr>";--}}
                                        ...
                                        @foreach($venta->articulos as $detalle)
                                            <tr id= "articulo{{$loop->index+1}}">
                                                ...
                                                <td data-name='del{{$loop->index+1}}'><button onclick='removeRowArticulo({{$loop->index+1}});' name='articulo{{$loop->index+1}}' class='btn btn-danger glyphicon glyphicon-remove row-remove'><span aria-hidden='true'>×</span></button></td>
                        ...
                        <table class="table table-bordered table-hover" id="tab_logic_total">
                            ...
                            <tr>
                                ...
                                <td class="text-center"><input type="number" name='sub_total' placeholder='0.00'
                                                               class="form-control" id="sub_total" value="{{$total}}" readonly/></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <td class="text-center"><input type="number" name='total_amount' id="total_amount"
                                                               placeholder='0.00' class="form-control" value="{{$total}}" readonly /></td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
...
{{--    --}}
{{--                                "</tr>";--}}
                                @foreach($venta->clientes as $detalle)
                                    <tr id= "cliente{{$loop->index+1}}">
                                        <td>{{$loop->index+1}}</td>
                                        <td class='col-md-7'>{{$detalle->nombreCompleto }}</td>
                                        <td class='col-md-1'> {{$detalle->NroVisitas}} </td>
                                        <td class='col-md-1' data-name='del{{$loop->index+1}}'><button onclick='removeRowCliente({{$loop->index+1}});' name='delcliente{{$loop->index+1}}' class='btn btn-danger glyphicon glyphicon-remove row-remove'><span aria-hidden='true'>×</span></button></td>
                                        <td style='display:none'> <input name='codigos_clientes[]' value='{{$detalle->IdCliente}}'> </td>
                                    </tr>
